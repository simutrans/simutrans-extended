...
sint64 karte_t::get_forge_cost(const waytype_t &waytype, const koord3d &position) const {
	...
	for (auto neighbour : koord::neighbours) {
		// note: lookup_kartenboden has its own is_within_limits check and will return NULL if not in limits
		const grund_t* gr_neighbour = lookup_kartenboden(neighbour + position.get_2d());
